...
export const PRESET_DESCRIPTIONS: Record<string, string> = {
  ...
  media:
    'The KB media substrate (ADR-0026): real `file`/`video` nodes whose bytes travel the product’s signed-upload transport into the private `kb-media` bucket (a `kmm` satellite + a bucket object both exist), a PRIVATE file owned by another user (the download RLS-negative), a file nested under an ancestor-shared folder (inherited-grant download positive), and a file per-user-granted to a node-only member without space-wide update (the read/write asymmetry — the read-grant allows download, but the write fence blocks upload). Bytes egress ONLY via short-lived signed URLs; `storage.objects` SELECT + satellite RLS is the read fence, and the WRITE fence composes NO grants (owner-or-space-update only, so a read-grantee cannot overwrite bytes).',
  search:
