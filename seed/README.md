...
so log in as `demo-admin` to see and edit it.
...
mechanism-distinction fixture: one non-owner `viewer` sees four nodes
...
fixture: the worked example for the tariff-gated ADVANCED (structural) display
...
so the SAME three nodes the flat digest lists are re-arranged structurally. The e2e
draws the tree via `seedAdvancedSharedFixture`; the COMMERCIAL entitlement rows are
...
The `shared` preset ALSO carries the `containment-inheritance` fixture: the
...
change): the access-matrix e2e draws the tree via `seedContainmentInheritanceFixture`
...
the grantee sees it, a third un-granted member stays blind). That ONE grant is
read from BOTH ends of the grant graph: the grantee sees the doc in the
...
Share dialog's co-member identity directory: the people-picker + "who has access"
...
paginated directory-v2 picker: a page of 5 + "+N more", a keyset
...
`search` is the lexical-search corpus (slice-12): the `knowledge-base` scenario
...
ranks title above description at equal tier) — PLUS the RLS-absence proof:
...
them via the inherited-grant disjunct composing through search. RLS is the SOLE
fence — there is no app-level visibility filter — so the search SELECT runs as the user
through the reused projection-resolve transport. The other-space negative
...
whole path renders.
`media` is the KB media substrate (slice-13; slice-14 resumable/TUS switch): the
...
whose bytes are genuine base64-decoded binaries — the inline MIME-driven PREVIEW
...
file nested under the ancestor-shared folder (`kb/inherited-file`, the inherited-grant
...
(the touch-item). Bytes egress ONLY via short-lived signed URLs; RLS is
the sole fence. The media matrix e2e (`knowledge-media-substrate.e2e.spec.ts`) draws
...
spec also drives the co-member directory through the shared `visibility`
...
the `visibility` wrapper now returns `members` as a keyset PAGE
...
(`knowledge-containment-inheritance.e2e.spec.ts`) likewise draws its whole
...
The lexical-search matrix spec (`knowledge-search.e2e.spec.ts`, slice-12)
...
a SECOND consumer of the search capability — the command palette — drives
...
The Drive size-&-filter render spec (`knowledge-drive-size-filter.e2e.spec.ts`)
draws its whole tree from the shared `drive-size-filter` fixture (an e2e-only
...
(`knowledge-search-size-filter.e2e.spec.ts`): the "Only files" chip now lives
...
The KB media matrix spec (`knowledge-media-substrate.e2e.spec.ts`, slice-13, the
...
also fails and the owner's bytes stay intact). It ALSO proves the PURGE REAP (the
