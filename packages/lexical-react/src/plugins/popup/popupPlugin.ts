import { isTouchDevice } from '../../support/environment';
...
import { ALL_TRANSFORMERS } from '@workspace/lexical-nodes';
import {
  $addUpdateTag,
  $createParagraphNode,
  $getEditor,
...
function $enhanceRangeSelection(
  ...
  const selectedNodes = selection.getNodes().map((node) => {
    return node.getKey();
  });
  ...
  // Rendering the selection to markdown builds throwaway nodes inside the
  // selection-change update. They never attach, but they are dirty, and
  // the history plugin would record the selection as a document change —
  // a phantom undo step after every selection. Fold it into the current
  // entry instead.
...
function $getSelectedRootChildNodes(
  ...
  const rootChildNodes = new Set<string>();
...
function registerPopupPlugin(editor: LexicalEditor, props: PopupPluginProps) {
  ...
  return mergeRegister(
    editor.registerCommand(
      SELECTION_CHANGE_COMMAND,
      () => {
        // The origin wrapped this in an explicit batch so the visibility
        // flip and the selection write landed in one update. React batches
        // updates from any context automatically, so the body runs directly.
