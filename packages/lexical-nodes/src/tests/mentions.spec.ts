...
describe('parsePullRequestMentions', () => {
  it('extracts the label from a PR mention', () => {
    const input =
      '<m-pr-mention>{"id":"foreign-1","label":"acme/repo#123 Fix bug"}</m-pr-mention>';
    expect(parsePullRequestMentions(input)).toBe('acme/repo#123 Fix bug');
  });
...
describe('markdownToPlainText', () => {
  ...
  it('handles text with PR mentions', () => {
    const input =
      'See <m-pr-mention>{"id":"foreign-1","label":"acme/repo#123"}</m-pr-mention>.';
    expect(markdownToPlainText(input)).toBe('See acme/repo#123.');
  });
...
describe('markdownToEmbeddingText', () => {
  ...
  it('keeps ids from PR mentions', () => {
    const input =
      'See <m-pr-mention>{"id":"foreign-1","label":"acme/repo#123"}</m-pr-mention>.';
    expect(markdownToEmbeddingText(input)).toBe(
      'See [acme/repo#123](pr:foreign-1).'
    );
  });
  ...
  it('reduces contact and theme mentions to their names', () => {
    const input =
      'Ping <m-contact-mention>{"contactId":"c1","name":"Ness Chu","emailOrDomain":"ness@example.com","isCompany":false}</m-contact-mention> re ' +
