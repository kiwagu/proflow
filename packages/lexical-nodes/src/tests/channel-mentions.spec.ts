...
describe('extractChannelMentionsFromMarkdown', () => {
  ...
  it('extracts user mentions and re-tags bot principals', () => {
    const markdown =
      '<m-user-mention>{"userId":"local|a@b.com","email":"a@b.com"}</m-user-mention> and ' +
      '<m-user-mention>{"userId":"bot|00000000-0000-0000-0000-00000000a1a1","email":"Assistant"}</m-user-mention>';
    expect(extractChannelMentionsFromMarkdown(markdown)).toEqual([
      { entityType: 'user', entityId: 'local|a@b.com' },
      {
        entityType: 'bot',
  ...
  it('extracts mentions across multiple lines and formatting', () => {
    const markdown = `# heading\n\n- a list item with ${documentMention('doc-9', 'email')}\n\nand a user <m-user-mention>{"userId":"local|a@b.com","email":"a@b.com"}</m-user-mention>`;
    expect(extractChannelMentionsFromMarkdown(markdown)).toEqual([
