 ...
 * container folder, `to` the child node (FORWARD).
 */
...
/**
 * One `shortcut` edge (folder → target) as the Drive view reads it.
...
export type KbAttributes = {
  ...
  /**
   * Real media satellite (`kb.resource_media_meta`) for `kind=file|video`
 ...
 * only. A tag IS an ordinary node, and "resource R has tag T"
...
export type NodeMeta = {
   ...
   * mere views), maintained by the recency roll-up. Drives the "Modified"
  ...
  /**
   * The node's BROADCAST FLOOR (`knowledge_resources.visibility`) — the
   ...
   * line — the read-side mirror of the ShareDialog's floor selector. A
 ...
 * per-node ownership to DISPLAY-GATE its items (gating = display). This is
...
export type SpaceCapabilities = {
  ...
  canCreate: boolean;
  /**
   * `space.knowledge.access` — the audience-management verb.
   ...
   * `canShare = owned || canAccess` — display courtesy, never the
 ...
 *
 * This is a DISPLAY gate, never a security boundary: the advanced
...
export type SpaceEntitlements = {
   ...
   * Starred + Trash) is included in this space's plan. ONE generic
 ...
 * Cohort grants compose additively on top. */
export type ResourceFloor = 'private' | 'space' | 'organization';

/** One per-user grant on a node — a person this resource is shared with,
 * with a display label resolved from the co-member directory. `email` is the
...
/** One grantable space member for the Share dialog's people-picker:
 ...
 * `email` is the secondary disambiguator line. The list is a UX convenience,
 * never the fence. */
...
/**
 * One audience entry of a "Shared by me" resource — a
 ...
 *                 labelled via the co-member directory; `userId` is their id.
...
/**
 * One entry of the "Shared by me" lens (extended for cohort-by-me) — ONE resource the CURRENT user has shared OUT, paired with the AUDIENCE
 ...
 *   panel can render a person vs a group; people via the co-member directory,
 ...
 * lens. The `'shared'` lens is "visible nodes I do NOT own" — but the
 * resolver (frozen, `security invoker`) returns visible nodes WITHOUT saying
 ...
 *   - `cohort`    — a cohort grant (`knowledge_resource_scopes`) to a
 ...
 *                   deliberate share TO me either); a later
...
/**
 * The "Shared with me" mechanism annotation — a map from a shared
...
/**
 * ONE page of the grantable-member picker. The directory function pages
